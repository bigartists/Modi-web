...
export const useGetDeployment = (params: any) => {
  ...
  const memoizedValue = useMemo(
    () => ({
      deployment: (data?.result as IDeploymentItem) || [],
      deploymentLoading: isLoading,
      deploymentError: error,
      deploymentValidating: isValidating,
      deploymentEmpty: !isLoading && !data?.result.length,
    }),
    [data?.result, error, isLoading, isValidating]
  );
  return memoizedValue;
...
export const useGetPod = (params: { ns: string; pod: string }) => {
  const URL = params
    ? [
        '/pod',
        {
  ...
  const memoizedValue = useMemo(
    () => ({
      pod: (data?.result as IPodItem) || [],
      podLoading: isLoading,
      podError: error,
      podValidating: isValidating,
      podEmpty: !isLoading && !data?.result?.length,
    }),
    [data?.result, error, isLoading, isValidating]
  );
  return memoizedValue;
