...
export function RenderCellStatus({ params }: ParamsProps) {
  return (
    <Label variant="soft" color={(params.row.IsComplete === true && 'info') || 'default'}>
      {params.row.IsComplete === true ? 'Active' : 'Waiting...'}
    </Label>
  );
}
...
export function RenderCellNsName({
  params,
  handleClick,
}: ParamsProps & {
  handleClick: () => void;
  ...
  return (
    <Stack direction="row" alignItems="center" sx={{ py: 2, width: 1 }}>
      <ListItemText
        ...
        secondary={
          <Box component="div" sx={{ typography: 'body2', color: 'warning.main' }}>
            {params.row.Message}
          </Box>
        }
